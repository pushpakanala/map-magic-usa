...
import { MapPin, Globe, Award, School, BookOpen, Users, Building2, GraduationCap, ChartPieIcon } from 'lucide-react';
import {
  Table,
  TableBody,
  TableCell,
...
const CollegePage = () => {
  ...
  const students = universityData?.data.students;
  const faculty = universityData?.data.faculty;

  const prepareChartData = (data) => {
    return Object.entries(data.race_ethnicity).map(([name, value]) => ({
      name,
      value
    }));
  };

  const studentDemographics = students ? prepareChartData(students) : [];
  const facultyDemographics = faculty ? prepareChartData(faculty) : [];
